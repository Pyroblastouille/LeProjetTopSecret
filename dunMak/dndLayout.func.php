...
function getAreaElement()
{
    $element = new \stdClass();
    $book = getPageTileTable(rollDice(10));
    switch ($book) {
        case "DB1":
            $element->pages = getPageVol1(rollDice(100));
            $element->size = array((count($element->pages))*12, 12);
            break;
        case "DB2":
            $element->pages = getPageVol2(rollDice(100));
            $element->size = array((count($element->pages))*12, 12);
            break;
        case "LBD":
            $element->pages = getLittleBook(rollDice(100));
            $element->size = array((count($element->pages)) * 6, 6);
            break;
        default:
            $element->pages = $book;
            $element->size = 0;
...
function getPosByPrevious($previous, $el, $total)
{
    do {
        $el->position = $previous->position;
        if (count($previous->pages) == 2) {
            if (count($el->pages) == 2) {
                switch (rollDice(7)) {
                    case 1:
                        $el->position[1] -= $el->size[1];
                        $el->position[0] -= $el->size[0]/2;
                        break;
                    case 2:
                        $el->position[1] -= $el->size[1];
                        break;
                    case 3:
                        $el->position[1] -= $el->size[1];
                        $el->position[0] += $previous->size[0]/2;
                        break;
                    case 4:
                        $el->position[0] += $previous->size[0];
                        break;
                    case 5:
                        $el->position[1] += $previous->size[1];
                        break;
                    case 6:
                        $el->position[1] += $previous->size[1];
                        $el->position[0] -= $el->size[0]/2;
    ...
    } while (collapsingWithOtherZone($el, $total));
    return $el->position;
}
function collapsingWithOtherZone($el, $allOther)
{
    $r1x = $el->position[0];
    $r1y = $el->position[1];
    $r1w = $el->size[0];
    ...
    foreach ($allOther as $otherEl) {
        $r2x = $otherEl->position[0];
        $r2y = $otherEl->position[1];
        $r2w = $otherEl->size[0];
        $r2h = $otherEl->size[1];
...
{
    $numberOfAreas = getNumberOfAreas(rollDice(20));
    $areas = array();

    for ($i = 0; $i < $numberOfAreas; $i++) {
        $areaSize = getAreaSize(rollDice(12));
        $areas[$i] = array();
        $areas[$i][0] = getAreaElement();
        $areas[$i][0]->position = array(0, 0);

        for ($x = 1; $x < $areaSize; $x++) {
            $ar = getAreaElement();

            $ar->position = getPosByPrevious(
                $areas[$i][$x - 1],
                $ar,
                $areas[$i]
            );
